 ...
 * @version 1.01 Date: 2017/03/24
 * 
 * @TestCaseID 1P
 * @TestArea Registration
 * @TestScenarioDesc Registration from eMag cross promotion (centrewell) section (L4 entry point)
 * @TestStep
 * 
 *     	1. From L4 entry point, click on "Register" link in the eMag cross promotion (centrewell) section
 ...
 *     	7. Check on any category checkboxes.
...
/**
 * This test case contain method to test user is able to register from eMag cross promotion (centrewell) section (L4 entry point)
 * @author Teng Shyh Wei
 *
 */
public class Registration_0016 extends SuiteBaseReg{
	...
	public void checkCaseToRun() throws IOException{
		...
		FilePath_TestResult = Registration_0016_TestResult;
	...
	@Test(dataProvider="Registration_0016Data")
	public void Registration_0016Test(String dataTestCaseName, 
		...
		try{		
			...
			// 1. From L4 entry point, click on the "Register" link in the eMag cross promotion (centrewell) section
			ProductInquiry_Action.ExecuteL4EntryPoint(dataProductKeyword);
			
			// Click on "Register" link in the eMag cross promotion (centrewell) section
			ProductListing_Page.lnk_clickRegister().click();
			...
			// 4. Enter a product keyword in PA textbox (e.g.: MP3)
			// 5. Make sure the Notification checkboxes are checked.
			Registration_Action.ExecuteReg_FillUp_eMag_PA_Notification(dataBusinessEmail, dataCreateAPassword, dataReenterPassword, dataTitle,
					dataFirstName, dataLastName, dataCountryTerritory, dataCompanyName, dataTelephoneNumber, dataRadbtnCompWebsiteURL, 
					dataTxtbxCompWebsiteURL, dataEmagTitle, dataProductKeyword, dataMailAnnouce, dataMailMarketing);
			...
			Registration_Action.ExecuteReg_PA_Notification_FilledUp(FilePath_TestResult, TestCaseName, DataSet+1, testFail, dataProductKeyword);
			
			// 6. Click on the "Register Now" button.
			BuyerRegistration_Page.btn_clickRegisterNow().click();
			
			// To verify Select Product Alert Categories page is displayed.
			// There are suggested categories for the product keyword entered in Step4.
			...
			// To verify that "You've also requested sourcing magazines." statement is not exists and the PA categories selected in Step 7 are displayed.
			RegConfirmation_Action.ExecuteRegConfPage_NoReqSMCopy_PACatSelIsDisplayed(FilePath_TestResult, TestCaseName, DataSet+1, testFail, checkedCatNameList);
			...
			// To verify the PA categories selected in Step 7 are displayed.
			ProductAlert_Action.ExecutePAHomepage_PACatSelIsDisplayed(driver, FilePath_TestResult, TestCaseName, DataSet+1, testFail, checkedCatNameList);
	 ...
	 */
	@DataProvider
	public Object[][] Registration_0016Data(){
