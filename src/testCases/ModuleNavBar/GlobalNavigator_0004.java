/**
 * This class contains a method to verified Global Nav for logged-in/cookied non SYP user (DOI-ed user)
 * Created: Mon 06 Mar 2017
 * @author Teng Shyh Wei
 * @version 1.01 Date: 2017/03/06
 * 
 * @TestCaseID 
 * @TestArea Global Nav for logged-in/cookied non SYP user (DOI-ed user)
 * @TestScenarioDesc Global Nav for logged-in/cookied non SYP user (DOI-ed user)
 * @TestStep
 * 
 * 		Pre-req: 1. Clear Cache & Cookie on the browser before starting the test:
 * 		2. User has already subscribe to PA and DOI-ed
 * 		Email: autoqa.palite1001@gmail.com
 * 		Password: asdf9876
 * 		
 * 		1. As a logged in user, go to GSOL PP page.
 * 		2. Mouseover the user's menu at Global Navigator.
 * 		3. Under Path + PA Link, verified that DOI message is not displayed.
 * 		4. Go to Supplier homepage.
 * 		5. Repeat step 2.
 * 
...
/**
 * This test case contain method to test Global Nav for logged-in/cookied non SYP user (DOI-ed user)
 * @author Teng Shyh Wei
 *
 */
public class GlobalNavigator_0004 extends SuiteBaseRFI{
	...
	public void checkCaseToRun() throws IOException{
		...
		FilePath_TestResult = GlobalNavigator_0004_TestResult;
	 ...
	 */
	@Test(dataProvider="GlobalNavigator_0004Data")
	public void GlobalNavigator_0004Test(String dataTestCaseName,
			String dataUsername,
			String dataPassword,
		...
		try{
			...
			// 1. Login as non SYP user and DOI-ed user, go to GSOL PP page.
		//	ProductInquiry_Action.ExecuteLoggedIn_GSOLPPEntryPoint(dataUsername, dataPassword, dataSearchKeyword);
			...
			// To verify DOI message is not displayed.
			// To verify User Profile, My Product Alert, My Preferred Exhibitors, Register as Supplier and Logout button are displayed.
			GlobalNav_MenuBar.check_DOIMsgNotExistsInGlobalNav(FilePath_TestResult, TestCaseName, DataSet+1, testFail);
			GlobalNav_MenuBar.check_subMenuNonSYPUserAvailable(FilePath_TestResult, TestCaseName, DataSet+1, testFail);
			
			// 3. Under Path + PA Link, verified that DOI message is not displayed.			
			// To verify DOI message is not displayed.
			ProductProfile_Page.check_DOIMsgUnderPALinkIsNotDisplayed(FilePath_TestResult, TestCaseName, DataSet+1, testFail);			
	 ...
	 */
	@DataProvider
	public Object[][] GlobalNavigator_0004Data(){
