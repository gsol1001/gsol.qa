/**
 * This class contains a method for Inquire Now for multiple verified products from single category
 ...
 * @TestStep
 * 
 * 		1. As a non-logged in user, 'inquire now' for multiple verified product from single category and go to RFI form. 
 ...
 * 		4. Fill in E-mail Address (autoqa_gsol1004@gmail.com) and Password (asdf9876) and click on "Login Now" button.
 * 		5. Make sure the PA upsell checkbox is unchecked and click on "Contact Supplier Now" button.
 * 
 ...
 * Product Inquiry - Verified, 1 PP/>1 PP, 1 Category, Inquire Now.
 * @author Teng Shyh Wei
 *
 */
public class InqFlowLI2_PdtInq_0005 extends SuiteBaseRFI{
	...
	public void checkCaseToRun() throws IOException{
		...
		FilePath = TestCaseListExcel_SuiteRFI_004_TestData;
		FilePath_TestResult = InqFlowLI2_PdtInq_0005_TestResult;
	...
	@Test(dataProvider="InqFlowLI2_PdtInq_0005Data")
	public void InqFlowLI2_PdtInq_0005Test(String 
			dataTestCaseName, 
		...
		try{
			...
			// 1. As a non-logged in user, 'inquire now' for single verified product from single category and go to RFI form.
			
			// To verify User's contact details are not pre-populated
	//		Home_Page.check_userNotLoggedIn(FilePath_TestResult, TestCaseName, DataSet+1, testFail);
        	        	
			// Enter search keyword by select Products Page Types
			ProductInquiry_Action.ExecuteSearchByProducts(dataSearchKeyword);
    		
			// 'inquire now' for multiple verified products from single category
			ProductInquiry_Action.ExecuteInqNow_MulVerPdt_SglCat(); 		
        	...
        	// 2.a. Entered some texts in "Subject", "Message" and "Country in which product(s) will be sold:" fields.
        	//   b. Check Both Verified and Unverified products Inquire All upsell checkbox.
    		InquiryForm_Action.ExecuteLI2_PdtInq_InqNow_MulVerPdt_SglCat_Info(dataSubject, 
    				dataExpOrderQuantity, dataRequestFor, dataMessage, dataInqAllVerUpsell, dataInqAllUnvUpsell, dataTargetCountry);
    		...
    		InquiryForm_Action.check_bothInqAllUpsellChkbxSelected(FilePath_TestResult, TestCaseName, DataSet+1, testFail);
    		

    		// To get category PA Upsell value before Login
    		String element = driver.findElement(By.xpath(Object.getProperty("inqL4Category"))).getText();
    		String getL4CategoryBeforeLogin = element.substring(4).trim();
    		Add_Log.info("Print {Inquired L4 Category} before login ::" + getL4CategoryBeforeLogin);
        	...
        	InquiryForm_Action.check_bothInqAllUpsellChkbxRemainSelected(FilePath_TestResult, TestCaseName, DataSet+1, testFail);
        	
			// To verify the category in PA upsell remains unchanged.
			String getL4CategoryAfterLogin = element.substring(4).trim();
			Add_Log.info("Print {Inquired L4 Category} after login ::" + getL4CategoryAfterLogin);
			if(getL4CategoryBeforeLogin.equals(getL4CategoryAfterLogin))
			{
				InquiryForm_Page.check_catPAUpsellRemainUnchangedIsTrue(FilePath_TestResult, TestCaseName, DataSet+1, testFail);				
			}else{
				InquiryForm_Page.check_catPAUpsellRemainUnchangedIsFalse(FilePath_TestResult, TestCaseName, DataSet+1, testFail);				
			}
			
						
        	// 5. Make sure the PA upsell checkbox is unchecked and click on "Continue to Next Step" button.
    	...
    	//	captureRFIConfirmationPageScreenShot(); 
    		...
    		RFIConfirmation_Page.check_PASection_UpdateMeOnNewL4Exists(FilePath_TestResult, TestCaseName, DataSet+1, testFail, getL4CategoryAfterLogin);
    	...
    	//	RFIConfirmation_Action.check_RFIUpsellSection_PdtSimilar_InqAllExists(FilePath_TestResult, TestCaseName, DataSet+1, testFail, getL4CategoryAfterLogin);
	 ...
	 */
	@DataProvider
	public Object[][] InqFlowLI2_PdtInq_0005Data(){
