/**
 * This class contains a method for RFI Attachment
 * Created: Thu 30 April 2015
 * @author Teng Shyh Wei
 * @version 1.01 Date: 2015/05/18
 * 
 * @TestCaseID 1191 / 32A
 * @TestArea RFI Attachment
 * @TestScenarioDesc Able to upload multiple attachments (Total size < 2MB)
 * @TestStep
 * 
 * 		1. Click on "Inquire Now" button of a product from Xxx Company / supplier - Xxx Company.
 * 		2. Input message.
 * 		3. Click on "Attach File(s)" link.
 * 		4. Select a file and click "Open".
 * 		5. Repeat steps 3 - 4 to attach more files.
 * 		6. Click on "Login Now".
 * 		7. Input login credentials (Email: autoqa_gsol1001@gmail.com / PW: asdf9876).
 * 		8. Click "Login Now".
 * 		9. Click "Contact Supplier Now".
 * 
...
/**
 * This test case contain method to test user able to upload multiple attachments (Total size < 2MB)
 * @author Teng Shyh Wei
 *
 */
public class RFI_Attachment_0001 extends SuiteBaseRFI{
	...
	public void checkCaseToRun() throws IOException{
		...
		FilePath_TestResult = RFI_Attachment_0001_TestResult;
	...
	@Test(dataProvider="RFI_Attachment_0001Data")
	public void RFI_Attachment_0001Test(String dataTestCaseName, 
		...
		//	ProductInquiry_Action.ExecuteSearchByProducts(dataSearchKeyword);
            ...
            // 2. Enter text for Message field
			InquiryForm_Page.txtbx_enterMessage().sendKeys(dataMessage);
			
            
            // 3. Click on "Attach File(s)" link            
			// 4. Select a file and click "Open".
			InquiryForm_Page.btn_uploadFirstFile();
            
			// To verify file is uploaded.
			InquiryForm_Page.check_fileIsUploaded(FilePath_TestResult, TestCaseName, DataSet+1, testFail);
			
            // 5. Repeat steps 3 - 4 to attach more files.
            // To verify files are uploaded.           
            InquiryForm_Action.ExecuteRFIAttachment_AttachMoreFiles(FilePath_TestResult, TestCaseName, DataSet+1, testFail);            
       			
			
            // 6. Click on the "Login Now" button and login to GSOL	
         	SignIn_Action.ExecuteLoginOnForm(driver, dataUsername, dataPassword);         	
        	
         	// To verify buyer's contact details are populated in the "Your Contact Details to Supplier" fields.
         	InquiryForm_Page.check_userContactDetailsPrepopulated(FilePath_TestResult, TestCaseName, DataSet+1, testFail, dataUsername);
         	
            // 9. Click 'Contact Supplier Now' button
	 ...
	 */
	@DataProvider
	public Object[][] RFI_Attachment_0001Data(){
